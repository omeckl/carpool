...
export default function MyPassengers({ goBack, backLabel, listingId }: MyPassengersProps) {
  ...
  const active = passengers.filter((p) => p.booking_status === "active");
  const cancelled = passengers.filter((p) => p.booking_status !== "active");
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {loading ? (
          ...
          <>
            ...
            {cancelled.length > 0 && (
              <div>
                <h2 className="text-sm font-bold text-[#717171] uppercase tracking-wider mb-3">Lemondott</h2>
                <div className="space-y-3">
                  {cancelled.map((p) => (
